...
This roadmap tracks pipeline work that addresses the [documented issues](EPSTEIN-FILES-ISSUES-AND-PIPELINE-SOLUTIONS.md) with the DOJ Epstein Files release.
...
- **Dry-run, PDF guardrails, batched DB transactions** — cost preview, oversize skip, concurrency limit, atomic loads.
- **Timeline + AI classification** — partial answer to Issue 5 (chronology) and Issue 8 (scale).
- **dedup-persons** — person-level dedup only; document-level is Priority 4.

## How to use this

- When starting work, set the row’s **Status** to `In progress` and add a **Notes** line (e.g. branch or design choice).
